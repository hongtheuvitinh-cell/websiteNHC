        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-3xl font-bold text-slate-800">
              ...
              {activeTab === 'youth_union' && 'Quản lý Hoạt động Đoàn'}
              {activeTab === 'achievements' && 'Quản lý Thành tích học tập'}
              {activeTab === 'schedule' && 'Quản lý Lịch công tác'}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                          ...
                          <div className="flex gap-2">
                            <select 
                              ...
                              className="flex-[2] p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-bold text-slate-700 bg-white"
                              required
                            >
                            ...
                            <select 
                              value={documentForm.grade}
                              onChange={e => setDocumentForm({...documentForm, grade: e.target.value})}
                              className="flex-1 p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-black text-blue-600 bg-white"
                              required
                            >
                            ...
                            />
                             <div className="relative">
                               <input 
                                 type="file" 
                                 accept="image/*,application/pdf"
                                 onChange={(e) => handleFileUpload(e, setDocumentForm, documentForm, 'file_url')}
                                 className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"
                               />
                               <button type="button" className="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-slate-100 transition-colors">
                                 <ImageIcon className="w-5 h-5" />
                               </button>
                             </div>
                          </div>
                          <label className="flex items-center gap-3 px-4 border rounded-xl bg-slate-50 cursor-pointer hover:bg-slate-100 transition-colors">
                            <input 
                              type="checkbox" 
                              ...
                              className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
                            />
                            <span className="text-sm font-black text-slate-700 uppercase tracking-widest">Tài liệu mới</span>
                          </label>
                        </div>
                        <textarea 
                          ...
                          className="p-3 border rounded-xl outline-none focus:ring-2 focus:ring-blue-500 h-20 w-full mb-4"
                        />
                        <div className="flex gap-3">
                          <button type="submit" className="px-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200">
                            {editingDocumentId ? 'Cập nhật tài liệu' : 'Thêm tài liệu'}
                          </button>
                          {editingDocumentId && (
                            <button 
                              type="button" 
                      ...
                      <div className="space-y-8">
                               ...
                               <div className="flex p-0.5 bg-slate-100 rounded-lg">
                                {[
                                  ...
                                  <button 
                                    key={f.id}
                                    type="button"
                                    onClick={() => setDocGradeAdminFilter(f.id as any)}
                                    className={`px-3 py-1 rounded-md text-[10px] font-black transition-all ${docGradeAdminFilter === f.id ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}
                                  >
                                    {f.label}
                                  </button>
                                ))}
                               ...
                               <div className="flex p-0.5 bg-slate-50 rounded-lg border border-slate-100">
                                {[
                                  ...
                                  <button 
                                    key={f.id}
                                    type="button"
                                    onClick={() => setDocStatusAdminFilter(f.id as any)}
                                    className={`px-3 py-1 rounded-md text-[10px] font-black transition-all ${docStatusAdminFilter === f.id ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400'}`}
                                  >
                                    {f.label}
                                  </button>
                                ))}
                        ...
                        {['Giáo án', 'Đề KT', 'Chuyên đề', 'Hệ thống học tập'].map(cat => {
                          const catDocs = deptDocuments.filter(d => 
                            d.category === cat && 
                            (docGradeAdminFilter === 'all' || d.grade === docGradeAdminFilter) &&
                            (docStatusAdminFilter === 'all' || (docStatusAdminFilter === 'new' ? d.is_new : !d.is_new))
                          );
                                    ...
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                        <h5 className="font-bold text-slate-800 line-clamp-1 flex items-center gap-1.5">
                                          <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100">K{d.grade === 'all' ? 'C' : d.grade}</span>
                                          {d.title}
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              <div className="flex justify-between items-center bg-blue-50 p-6 rounded-2xl border border-blue-100 shadow-sm">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                     <h3 className="text-xl font-black text-blue-900">Danh sách phản hồi</h3>
                     <p className="text-sm text-blue-600 font-medium">Bạn có {contactSubmissions.length} tin nhắn từ người dùng trang web</p>
                   </div>
                </div>
                <button 
                  onClick={() => fetchData()}
                  className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-blue-200 text-blue-600 font-bold hover:bg-blue-600 hover:text-white transition-all shadow-sm active:scale-95"
                >
                  <Zap className="w-4 h-4" /> Làm mới
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                {loading ? (
                  <div className="flex flex-col items-center justify-center p-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <p className="text-slate-400 font-bold animate-pulse">Đang tải hộp thư...</p>
                  </div>
                ) : contactSubmissions.length > 0 ? (
                  contactSubmissions.map((msg) => (
                    <div key={msg.id} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all group relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-6">
                        <div className="flex gap-4">
                          ...
                          <div className="space-y-1">
                            <h4 className="text-xl font-black text-slate-900 leading-none">{msg.name}</h4>
                            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 font-bold uppercase tracking-wider">
                               <span className="flex items-center gap-1.5 text-blue-600"><Mail className="w-3.5 h-3.5" /> {msg.email}</span>
                               <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {new Date(msg.created_at).toLocaleString('vi-VN')}</span>
                            </div>
                          </div>
                        </div>
                        <button 
                          onClick={() => handleDeleteMessage(msg.id)}
                          className="p-3 text-slate-300 hover:text-red-600 bg-slate-50 hover:bg-red-50 rounded-2xl transition-all opacity-0 group-hover:opacity-100 shadow-sm"
                          title="Xóa phản hồi"
                        >
                          <Trash2 className="w-5 h-5" />
                        </button>
                      ...
                      <div className="relative">
                        <div className="p-6 bg-slate-50 rounded-2xl text-slate-700 whitespace-pre-wrap text-base font-medium leading-relaxed border border-slate-100 italic relative z-10">
                          "{msg.message}"
                        </div>
                        <div className="absolute -bottom-4 -right-2 text-slate-100 scale-[4] opacity-20 pointer-events-none select-none -z-0">
                  ...
                  <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-slate-300 shadow-inner">
                     <Mail className="w-20 h-20 text-slate-100 mx-auto mb-6" />
                     <h4 className="text-xl font-black text-slate-400 mb-2">Hộp thư đang trống</h4>
                     <p className="text-slate-400">Chưa có phản hồi nào được gửi đến từ phía người dùng.</p>
                  </div>
                )}
              </div>
            </div>
