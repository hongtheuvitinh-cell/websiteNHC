...
export const SystemManager: React.FC<AdminManagerProps> = ({
  ...
  const handleBulkUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        newImages.push({ url: publicUrl, caption: file.name });
      }
      setGalleryImages([...galleryImages, ...newImages]);
      showSuccess(`Đã tải lên ${files.length} ảnh`);
    } catch (error: any) {
      showAlert("Lỗi", error.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-8">
      ...
      {activeTab === 'gallery' && (
        <div className="space-y-8">
           <form onSubmit={handleSaveGallery} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
              <div className="flex items-center gap-3 mb-4 bg-blue-50 p-4 rounded-2xl border border-blue-100">
                <ImageIcon className="w-6 h-6 text-blue-600" />
                <div>
                  <h3 className="text-lg font-black text-slate-800">{editingGalleryId ? 'Hiệu chỉnh album' : 'Thêm bộ ảnh mới'}</h3>
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">Quản lý kho ảnh nhà trường</p>
                </div>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-widest">Tiêu đề album</label>
                  <input 
                    type="text" 
                    placeholder="Ví dụ: Lễ khai giảng năm học 2024-2025" 
                ...
                <div>
                  <label className="block text-sm font-black text-slate-700 mb-2 uppercase tracking-widest">Link ảnh bìa (Thumbnail)</label>
                  <div className="flex gap-2">
                    <input 
                      type="url" 
                      placeholder="URL ảnh bìa hiện ra ngoài danh sách" 
                      value={galleryForm.image_url} 
                      onChange={e => setGalleryForm({...galleryForm, image_url: e.target.value})} 
                      className="flex-1 p-4 border-2 border-slate-100 rounded-2xl outline-none focus:border-blue-500 transition-all font-bold" 
                      required 
                    />
                    <label className="p-4 bg-slate-100 text-slate-600 rounded-2xl cursor-pointer hover:bg-slate-200 flex items-center justify-center transition-colors">
                       <Upload className="w-5 h-5" />
                       <input type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, setGalleryForm, galleryForm, 'image_url')} />
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                <div className="flex justify-between items-center mb-6">
                   <div>
                     <h4 className="font-black text-slate-800 text-sm uppercase tracking-widest">Danh sách ảnh trong album ({galleryImages.length})</h4>
                     <p className="text-xs text-slate-400 font-medium">Bạn có thể thêm nhiều ảnh bằng link hoặc tải lên trực tiếp</p>
                   </div>
                   <div className="flex gap-2">
                      <button 
                        type="button" 
                        onClick={handleAddImageUrl}
                        className="px-4 py-2 bg-white border border-slate-200 text-slate-600 text-xs font-black rounded-xl hover:bg-slate-100 transition-all flex items-center gap-2"
                      >
                        <Zap className="w-4 h-4 text-orange-500" /> Thêm từ Link
                      </button>
                      <label className="px-4 py-2 bg-blue-600 text-white text-xs font-black rounded-xl hover:bg-blue-700 transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-blue-100">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                   ...
                   {galleryImages.length === 0 && (
                     <div className="col-span-full py-10 flex flex-col items-center justify-center text-slate-400 italic">
                        <ImageIcon className="w-10 h-10 mb-2 opacity-20" />
                        <p className="text-sm font-bold">Chưa có ảnh nào trong album</p>
                     </div>
                   )}
                </div>
              </div>
              ...
              <div className="flex gap-4 pt-4">
                <button type="submit" className="px-12 py-4 bg-blue-600 text-white font-black uppercase tracking-[0.2em] rounded-2xl hover:bg-blue-700 transition-all active:scale-95 shadow-xl shadow-blue-200">
                  {editingGalleryId ? 'Cập nhật Album' : 'Tạo Album ngay'}
                </button>
                {editingGalleryId && (
                  <button 
                    type="button" 
                    ...
                    className="px-8 py-4 bg-slate-200 text-slate-600 font-black uppercase tracking-widest rounded-2xl hover:bg-slate-300 transition-all"
                  >
                    Hủy bỏ
                  </button>
                )}
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {gallery.map(item => (
                 <div key={item.id} className="bg-white rounded-3xl border border-slate-200 overflow-hidden relative group aspect-[4/5] shadow-sm hover:shadow-xl transition-all hover:-translate-y-1">
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end min-h-[50%]">
                       ...
                       <div className="flex items-center gap-2 mb-4">
                          <span className="px-2 py-0.5 bg-blue-600 text-white text-[10px] font-black rounded uppercase tracking-widest">
                            {JSON.parse(item.images_json || '[]').length} Ảnh
                          </span>
                       </div>
                       <div className="flex gap-2">
                          <button 
                            onClick={() => { 
                              ...
                              setGalleryForm({ ...item, images_json: item.images_json || '[]' }); 
                              try { 
                            ...
                            className="flex-1 py-2 bg-white/20 hover:bg-white text-white hover:text-slate-900 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all backdrop-blur-md"
                          >
                            <Edit className="w-4 h-4" /> Hiệu chỉnh
                          </button>
                          <button 
                            onClick={() => handleDeleteGallery(item.id)} 
                            className="w-10 h-10 bg-red-500/80 hover:bg-red-600 text-white rounded-xl flex items-center justify-center transition-all backdrop-blur-md"
                          >
